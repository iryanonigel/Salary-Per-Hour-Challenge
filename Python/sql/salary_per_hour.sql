...
WITH unique_employee AS (
	...
	FROM (
		SELECT
			*,
			ROW_NUMBER() OVER(PARTITION BY employe_id ORDER BY ingest_from DESC, salary DESC) AS num 
		FROM 
			di.employees
	) e
	WHERE num = 1 -- get the latest ingest file data with the biggest salary
)
